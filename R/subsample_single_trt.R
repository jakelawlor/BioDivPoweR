#' Subsample bootstrapped communities to assess effect of sampling effort on detectable effect size
#'
...
subsample_single_trt <- function(data,
  ...
  # n_to_check_comm1, and n_to_check_comm2, which list the number
  ...
  data3 <- data2 %>%
    dplyr::mutate(
      ...
      comm2_rich = purrr::map2(.x = comm2_matrix,
                        .y = n_to_check_comm2,
                        .f = ~ {
                          # explicitly name the arguments for clarity
                          comm_matrix <- .x
  ...
  # actual number of samples needed in the empirical community.

  # a. find average species occupancy in pilot survey
  pilot_occupancy <- colMeans(pilot[,-c(1)])

  # b. find coverage at 1:1000 samples in pilot survey
  pilot_coverage <- find_coverage(pilot_occupancy)

  # c. find target detection rate,
  # or the coverage needed to detect all species in the community
  pilot_target_detection_rate <- find_target_detection_rate(pilot)

  # d. rescale the pilot coverage from 0-target detection rate, instead of 0-100
  pilot_coverage_rescale <- pilot_coverage/pilot_target_detection_rate
  ...
  pilot_n_sites <- BioDivSampler:::find_eff_sites(pilot_coverage_rescale, coverage_seq)
  ...
  if(!is.null(cost_per_sample)){
    ...
    # calculate cost
    cost <- (sec_axis_labels * cost_per_sample)

    # add dollar signs, and original labels underneath
    sec_axis_labels <- paste0(scales::dollar(round(cost,0)),"\n", sec_axis_labels)
  ...
  pilot_approx_coverage_rank <- approx(x = coverage_seq,
                                       y = c(0:40),
                                       # achieved_pilot_coverage
                                       xout = pilot_coverage_rescale[nrow(pilot)])$y
  ...
  pilot_approx_effect_size <- purrr::map(
    .x = power_mod,
    .f = ~predict(.x,
                  list(coverage_rank = pilot_approx_coverage_rank))
  )%>%
    dplyr::bind_rows(.id = "power") %>%
  ...
  p3 <- p3 +
    ...
    ggplot2::annotate(geom = "segment",
             x = pilot_approx_coverage_rank,
             xend = pilot_approx_coverage_rank,
             y = Inf,
             yend = -Inf,
             linetype = "dashed") +
  ...
  if(!is.null(target_eff_size)){
    ...
    sample_n_to_reach_target <- approx(
      unique(power_au$coverage_rank),
      pilot_n_sites,
    ...
    ann <- coverage_rank_to_reach_target %>%
      dplyr::left_join(coverage_value_to_reach_target,
                by = dplyr::join_by(power, target_eff_size, coverage_rank)) %>%
      dplyr::left_join(sample_n_to_reach_target,
                by = dplyr::join_by(coverage_rank)) %>%
      dplyr::mutate(ann = n_samples)

    # if cost is supplied, add cost to annotation
    if(!is.null(cost_per_sample)) {

      # approximate cost from number of samples needed
      cost_to_reach_target <- approx(
        unique(power_au$coverage_rank),
        cost,
        coverage_rank_to_reach_target$coverage_rank
      ) %>% tibble::as_tibble() %>%
  ...
  out.df <- pilot_approx_effect_size %>%
    dplyr::mutate(group = "achieved",
                  n_samples = nrow(pilot)) %>%
    dplyr::rename(min_detectable_eff_size = achieved_min_eff_size)
